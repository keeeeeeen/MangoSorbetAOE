...
_pending_slices: list[tuple[str, int]] = []   # [(direction, timestamp_ms), ...]

def get_pending_slices() -> list[tuple[str, int]]:
    """
    Returns all slice inputs that arrived this frame as (direction, ms) pairs.

    KEYBOARD (current):  Filled by _handle_keyboard_event() below.

    IMU REPLACEMENT:     Delete the _pending_slices global and this function body,
                         then replace with a serial read — see IMU_HOOK B for the
                         exact code snippet.
    """
    global _pending_slices
    slices = _pending_slices[:]
    _pending_slices.clear()
    return slices
...
#
#  That's it — the entire rest of the game stays identical.
# ══════════════════════════════════════════════════════════════════════════════
...
class WandSlicer:
    ...
    def handle_slice(self, direction: str, timestamp_ms: int):
        ...
        dt = now - best.target_time   # positive = late
